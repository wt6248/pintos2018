...
void
thread_set_priority_donated(int new_priority)
{
	struct thread *cur = thread_current();
	int priority_old;
	ASSERT(!intr_context());
...
bool is_latter_priority_smaller(const struct list_elem *inserted, const struct list_elem *before, void UNUSED)
{
...
/* priority 큰게 앞으로.
list_insert_ordered (*list, struct &thread->elem,
					 is_latter_priority_smaller, 0)
